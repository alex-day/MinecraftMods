...
public class ComponentVillageField extends ComponentVillage
{
    ...
    /** NODE: added to allow additional crops to be spawned in villages */
    public static ArrayList<Integer> cropBlockIDs = new ArrayList<Integer>();
    static {
    	cropBlockIDs.add(Block.carrot.blockID);
    	cropBlockIDs.add(Block.potato.blockID);
    	cropBlockIDs.add(Block.crops.blockID);
    }
     ...
     * NODE: modified to use cropBlockID's
     */
    private int getRandomCrop(Random par1Random)
    {
    	int index = par1Random.nextInt(cropBlockIDs.size());
    	return cropBlockIDs.get(index);
    }
