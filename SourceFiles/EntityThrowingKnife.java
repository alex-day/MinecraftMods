...
public abstract class EntityThrowingKnife extends Entity implements IProjectile {
    ...
    public EntityThrowingKnife(World par1World, EntityLiving par2EntityLiving, EntityLiving par3EntityLiving, float par4, float par5, int type)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityThrowingKnife(World par1World, EntityLiving par2EntityLiving, float par3, int type)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLiving;
    ...
    public void onUpdate()
    {
        ...
        else
        {
                ...
                if (var4.entityHit != null)
                {
                    ...
                    if (var4.entityHit.attackEntityFrom(var21, var23))
                    {
                        ...
                        if (!(var4.entityHit instanceof EntityEnderman))
                        {
	                        this.setDead();
	                        if (!this.worldObj.isRemote) {
		                        //drop a knife item stack
		                        ItemStack itemStack = this.getAmmoItemStack();
                    ...
                    {
                    	if(!(var4.entityHit instanceof EntityThrowingKnife) && var4.entityHit != this.shootingEntity) {
                            if (!this.worldObj.isRemote) {
                            	//drop a knife item stack
                            	ItemStack itemStack = this.getAmmoItemStack();
                            	EntityItem entityItem = new EntityItem(worldObj, this.posX, this.posY, this.posZ, itemStack);
            ...
            this.motionY -= (double)var11/1.5f; //this is gravity
            this.setPosition(this.posX, this.posY, this.posZ);
	...
	protected void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
        ...
        this.knifeType = par1NBTTagCompound.getShort("knifeType");
	...
	protected void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
        ...
        par1NBTTagCompound.setShort("knifeType", (short)this.knifeType);
	}
    ...
    public void onCollideWithPlayer(EntityPlayer par1EntityPlayer)
    {
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0)
        {
            ...
            if (this.canBePickedUp == 1 && !par1EntityPlayer.inventory.addItemStackToInventory(this.getAmmoItemStack()))
            {
                var2 = false;
            }
    ...
    public void setKnifeType(int type) {
    	this.knifeType = type;
    }
    
    public int getKnifeType() {
    ...
    public ItemStack getAmmoItemStack() {
    	if(this.knifeType == ItemThrowingKnife.TYPE_IRON) {
    		return new ItemStack(mod_Alex.itemIronThrowingKnife, 1);
    	} else
    	if(this.knifeType == ItemThrowingKnife.TYPE_DIAMOND) {
    		return new ItemStack(mod_Alex.itemDiamondThrowingKnife, 1);
    	} else
    	if(this.knifeType == ItemThrowingKnife.TYPE_QUARTZ_RUBY) {
    		return new ItemStack(mod_Alex.itemQuartzRubyThrowingKnife, 1);
    	} else
    	if(this.knifeType == ItemThrowingKnife.TYPE_QUARTZ_AMETHYST) {
    		return new ItemStack(mod_Alex.itemQuartzAmethystThrowingKnife, 1);
    	}
    	return null;
    }
